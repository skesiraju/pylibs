...
class MCLR(nn.Module):
    ...
    def __init__(self, dim, n_classes, lam_w, cuda: bool=False):
        ...
        self.b = nn.Parameter(torch.randn(1, n_classes) * 0.001,
                              requires_grad=True)
        self.W = nn.Parameter(torch.randn(dim, n_classes) * 0.001,
                              requires_grad=True)
    ...
    def loss(self, logits, Y):
        ...
        return xen + self.reg_weights()

    def fit(self, X, Y, opt, trn_iters):
        """ Fit or train the model """

        if opt  == 'adam':
            print("Using adam with default learning rate.")
            opt = torch.optim.Adam([self.W, self.b])

        train(self, opt, X, Y, trn_iters)

    def fit_and_validate(self, x_train, y_train, x_dev, y_dev, opt='adam', out_sfx="",
                         trn_iters=100, val_iters=1, save=True):

        if opt == 'adam':
            print("Using adam with default learning rate.")
            opt = torch.optim.Adam([self.W, self.b])

        train_and_validate(self, x_train, y_train, x_dev, y_dev, opt, out_sfx,
                           trn_iters, val_iters, save)
...
class MCLRU(nn.Module):
    ...
    def __init__(self, dim, n_classes, lam_w=1e-4, R=32, cuda=False, use_uncert=True):
        """ initialize the model
    ...
    def sample(self, N):
        ...
        return torch.randn(size=(self.R, N, self.dim)).to(
            device=self.device)
    ...
    def loss(self, logits, Y):
        ...
        xen = torch.Tensor([0.]).to(device=self.device)
        if self.use_uncert:
    ...
    def fit(self, X, Y, opt='adam', trn_iters=100):
        """ Fit or train the model """
...
def train(model, optim, X, Y, trn_iters):
    ...
    for i in range(trn_iters):
        ...
        info_str = "Iter {:4d} / {:4d} Loss: {:f}".format(i+1, trn_iters, xen.detach().cpu().item())
...
def train_and_validate(model, x_train, y_train, x_dev, y_dev, optim, out_sfx,
                       trn_iters, val_iters=1, save=True):
    ...
    for i in range(trn_iters):
        ...
        if (i+1) % val_iters == 0:
            ...
            scores[i, 0] = np.mean(
                predict(model, x_train) == y_train.cpu().numpy()) * 100
            scores[i, 1] = np.mean(
                predict(model, x_dev) == y_dev.cpu().numpy()) * 100
...
def parse_arguments():
    ...
    parser.add_argument("-lw", default=1e-4, type=float,
                        help='L2 reg weight for the model params')
    parser.add_argument("-R", default=1, type=int,
                        help='number of samples for Monte Carlo approx')
